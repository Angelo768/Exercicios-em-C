...
void hand_out(stack_top *stack[], stack_top *stock, card *deck){
    ...
    stack_push(stack[0], deck[0].number, deck[0].suit, deck[0].color, deck[0].turn_on);
                           
    for(int i = 1; i < 3; i++) stack_push(stack[1], deck[i].number, deck[i].suit, deck[0].color, deck[0].turn_on);
    
    for(int i = 3; i < 6; i++) stack_push(stack[2], deck[i].number, deck[i].suit, deck[0].color, deck[0].turn_on);
    
    for(int i = 6; i < 10; i++) stack_push(stack[3], deck[i].number, deck[i].suit, deck[0].color, deck[0].turn_on);
    
    for(int i = 10; i < 15; i++) stack_push(stack[4], deck[i].number, deck[i].suit, deck[0].color, deck[0].turn_on);
    
    for(int i = 15; i < 21; i++) stack_push(stack[5], deck[i].number, deck[i].suit, deck[0].color, deck[0].turn_on);
    
    for(int i = 21; i < 28; i++) stack_push(stack[6], deck[i].number, deck[i].suit, deck[0].color, deck[0].turn_on);
    
    for(int i = 28; i < 52; i++) stack_push(stock, deck[i].number, deck[i].suit, deck[0].color, deck[0].turn_on);
}
